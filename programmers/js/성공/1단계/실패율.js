...
function solution(N, stages) {
  ...
  const arriveUsers = (stage) => {
    let sum = 0;
    for (let i = stage; i <= N + 1; i++) {
  ...
  stages.forEach((stage) => {
    counts[stage] ? counts[stage]++ : (counts[stage] = 1);
  });

  // 실패율 구하기
  const rates = Object.keys(counts).map((stage) => {
    // 전체 도달 수 구하기
    const users = !!arriveUsers(stage) ? arriveUsers(stage) : 0;
