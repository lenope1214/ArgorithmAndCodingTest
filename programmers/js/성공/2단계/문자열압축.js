...
function solution(s) {
  ...
  for (let n = 1; n < s.length; n++) {
    console.log("n :", n);
    // let last = s.substr(0, n);
    let last = "";
    let str = "";
    for (let i = 0; ; i += n) {
      let now = s.substr(i, n);
      console.log("last :", last);
      console.log("now :", now);
      console.log("last.substring(0, n) :", last.substring(0, n));
      if (last.substring(0, n) === now) {
        last += now;
      } else {
        console.log("last.substring(0, n) :", last.substring(0, n));
        const length = last.split(last.substring(0, n)).length - 1;
        str += (length > 1 ? length : "") + last.substring(0, n);
        last = now;
        // console.log("str :", str);
      }
      if (i > s.length) break;
...
let ans;
ans = solution("aabbaccc"); //7
console.log("ans :", ans, ans === 7);

ans = solution("ababcdcdababcdcd"); //9
console.log("ans :", ans, ans === 9);
ans = solution("abcabcdede"); //8
console.log("ans :", ans, ans === 8);
ans = solution("abcabcabcabcdededededede"); //14
console.log("ans :", ans, ans === 14);
ans = solution("xababcdcdababcdcd"); //17
console.log("ans :", ans, ans === 17);
("");
